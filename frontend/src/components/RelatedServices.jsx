import { useContext, useEffect, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { AppContext } from '../context/AppContext';

const RelatedServices = ({ category, serviceId }) => {
    const { services } = useContext(AppContext);
    const [relServices, setRelServices] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        if (services.length > 0 && category) {
            const serviceData = services.filter(
                (service) => service.category === category && service._id !== serviceId
            );
            setRelServices(serviceData);
        }
    }, [services, category, serviceId]);

    return (
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
            <h1 className='text-3xl font-medium'>Related Services</h1>
            <p className='sm:w-1/3 text-center text-sm'>
                Explore our range of vehicle maintenance and repair services to keep your car running smoothly. Book your service appointment effortlessly.
            </p>

            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {relServices.slice(0, 5).map((item, index) => (
                    <div onClick={() => { navigate(`/service/${item._id}`); scrollTo(0, 0) }} key={index} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer'>
                        <img className='bg-blue-50' src={item.displayImage} alt={item.serviceName} />
                        <div className='p-4'>
                            {
                                item.available ?
                                    <div className='flex items-center gap-2 text-sm text-center text-green-500'>
                                        <p className='w-2 h-2 bg-green-500 rounded-full'></p>
                                        <p>Available</p></div> :
                                    <div className='flex items-center gap-2 text-sm text-center text-red-500'>
                                        <p className='w-2 h-2 bg-red-500 rounded-full'></p><p>Not Available</p></div>
                            }
                            <p className='text-gray-900 text-lg font-medium'>{item.serviceName}</p>
                            <p className='text-gray-600 text-sm'>{item.category}</p>
                        </div>
                    </div>
                ))}
            </div>

            <button onClick={() => { navigate('/services'); scrollTo(0, 0) }} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10'>
                More
            </button>
        </div>
    )
}

export default RelatedServices;


